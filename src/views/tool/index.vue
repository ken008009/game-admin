        ...
        <el-table v-loading="loading" :data="list" size="small" empty-text="暂无数据" style="width: 100%; margin-top: 20px;">
          ...
          <el-table-column label="道具类型" align="center">
            <template slot-scope="scope">
              {{ toolList[scope.row.propType] }}  <!-- 从1开始而不是0 -->
            </template>
          </el-table-column>
          <!-- <template slot-scope="scope">
            <el-button type="primary" class="btn-option" size="mini">{{ scope.$index + 1 }}</el-button>
          </template> -->
          <el-table-column label="最大使用次数" prop="max" align="center" />
          ...
          >
            <template slot-scope="scope">
              <el-button v-if="(scope.row.propType !=='17')" type="primary" class="btn-option" size="mini" @click="editHandler(scope.row, scope.$index)">编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
import { toolList } from '@/api/orderModel'
